...
<template>
    ...
    <div class="pwfvf-scheduler"> <!--scheduler parent -->
        ...
        <div class="pwfvf-scheduler-dateboxes">
            ...
            <div>FRI</div>
            <div>SAT</div>
            <div @click="cb.onclick(cb.date)" class="pwfvf-scheduler-datebox" v-for="cb, i in calendarBoxes"
                :class="{ notCurrentMonth: !cb.isCurrentMonth, active: this.dateAdjusted(cb.date + ' 00:00:00').getTime() == this.dateAdjusted(qd.y, qd.m, qd.d).getTime(), hasSchedule: availDates.includes(cb.date) }" :key="i">
                <span>{{ cb.dateNum }}</span>
            </div>
        </div>
        <div class="pwfvf-scheduler-availscheds">
            <div class="spinner" v-if="fetching"></div>
            <div class="pwfvf-scheduler-availscheds-empty" v-if="!fetching && availableSchedules.length == 0">No
                schedules for this date...</div>
            <div class="pwfvf-scheduler-availscheds-item" v-for="asc in availableSchedules" @click="selectSchedule(asc)"
                :key="asc.book_schedule_id" :class="{ active: chosenSchedule == asc.book_schedule_id }">
                <h2>{{ asc.book_schedule_service }}
                    ...
                    <span v-if="asc.book_schedule_special_status != 0">{{
                        ['None', 'Reserved', 'Closed'][asc.book_schedule_special_status] }}</span>
                ...
                <small v-if="new Date(asc.book_schedule_date).getTime() > new Date('2024-11-01').getTime()"
                    class="text-sm">
                    {{ moment(asc.book_schedule_timestart, "H:mm").format("h:mma") }} -
                    {{ moment(asc.book_schedule_timeend, "H:mm").format("h:mma") }}
                </small>
                <small v-else class="text-sm">
                    {{ dateFormat('%h:%I%a', asc.book_schedule_date + ' ' + asc.book_schedule_timestart) }} -
                    {{ dateFormat('%h:%I%a', asc.book_schedule_date + ' ' + asc.book_schedule_timeend) }}
                </small>
            </div>
        </div>
