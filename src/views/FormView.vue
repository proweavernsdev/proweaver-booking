...
<template>
    <ProweaverForms v-if="onlinePayment.opened && onlinePayment.trans_id == ''"
        :onclose="() => { onlinePayment.opened = false }" :onsuccess="transId => {
    ...
    <div id="pwfv-parent" ref="formElement" v-if="form != null" data-responsive="" :data-page-index="currentPageIndex">
        ...
        <div class="pwfv-body" v-if="!formSubmitted">
            <div class="pwfv-navigation" v-if="form.pages.length > 1">
                <div class="pwfv-navigation-item" :class="{ active: currentPageIndex == i, done: progressIndex > i }"
                    v-for="p, i in form.pages" :key="i" @click="beforePageChange(null, i)">
                    <span>{{ progressIndex > i ? '&check;' : i + 1 }}</span>
                    {{ p.page_title }}
                </div>

                <div class="pwfv-holidays">
                    <h2>Federal Holidays 2024</h2>
                    <div class="pwfv-holidays-table">
                        ...
                        <div>Monday, January 15, 2024</div>
            ...
            <div class="pwfv-maingrid " :class="{ 'two-cols': currentPage.page_columns == 2 }">
                <p class="pwfv-required-reminder" v-if="currentPageRequired.length > 0">Required Fields are marked with
                    (*)</p>
                <div class="pwfv-maingrid-1">

                    <div class="pwfv-fielditem" v-for="f, i in filteredByColumn(1)"
                        v-show="f != null && (f.hidden == undefined || f.hidden == false)">
                        ...
                        <div v-if="f.content_type == 'rbfield' && !omittedRBFields.includes(f.id)">
                            ...
                            <RequestBindedFields :endpoint="f.endpoint" :based="f.based" :type="f.type"
                                :value="getFieldValue(f.id)" :readonly="f.readonly" @onResult="e => organizeInput(f, e)"
                                @onEmpty="deleteRBField(f.id)" />
                        </div>
                        <div v-if="f.content_type == 'scheduler'">

                            <label class="pwfv-fieldlabel">{{ f.text }} <span>*</span></label>
                            <SchedulerSelect :schedule="getFieldValue(f.id)"
                                :serviceSelect="selectedServiceFromScheduler" @onFetch="e => fetchingSchedules(e)"
                                @selectedService="e => selectedService(e)" @onResult="e => organizeInput(f, e)" />
                        </div>
                        ...
                        <div v-if="f.content_type == 'field' && !['checkbox', 'paypal'].includes(f.type)">
                            <label class="pwfv-fieldlabel">{{ f.label }} <span
                                    v-if="f.required && f.label != ' '">*</span></label>
                            <TemplatedFields :type="f.type" :name="f.name" :columns="f.grid" :readonly="f.readonly"
                                :required="f.required" :placeholder="f.placeholder" :values="f.values"
                                :select="f.type == 'checkbox-group' ? getFieldValue(f.id) : null" :options="f.options"
                                :index="f.index" :value="getFieldValue(f.id)" @onResult="e => organizeInput(f, e)" />
                        </div>
                        <div v-if="f.content_type == 'field' && f.type == 'checkbox'">
                            <TemplatedFields :type="f.type" :name="f.name" :columns="f.grid" :readonly="f.readonly"
                                ...
                                :select="f.type == 'checkbox-group' ? getFieldValue(f.id) : null" :options="f.options"
                                :index="f.index" :value="getFieldValue(f.id)" @onResult="e => organizeInput(f, e)" />
                            <label :for="f.name" class="pwfvf-checkbox-label">{{ f.label }} <span
                                    v-if="f.required">*</span></label>
                        </div>
                        <div v-if="f.content_type == 'field' && f.type == 'paypal' && !refreshPayPal">

                            <PayPalButtons :service="selectedServiceFromScheduler"
                                :fieldData="JSON.parse(JSON.stringify(f))" :paid="getFieldValue(f.id)"
                                :currency="form.declare.paypalCurrency" :paymentFunc="paidPaypal"
                                :paymentFuncParams="[f]" @onEmpty="() => deletePaypalAsPayment(f)"
                ...
                <div class="pwfv-maingrid-2" v-if="currentPage.page_columns == 2">
                    <div class="pwfv-fielditem " v-for="f, i in filteredByColumn(2)"
                        v-show="f != null && (f.hidden == undefined || f.hidden == false)" :key="i">
                        <!-- field renderer start-->
                        <div v-if="f.content_type == 'rbfield' && !omittedRBFields.includes(f.id)">
                        ...
                        <div v-if="f.content_type == 'scheduler'">
                            ...
                            <label class="pwfv-fieldlabel">{{ f.text }} <span>*</span></label>
                            <SchedulerSelect :schedule="getFieldValue(f.id)" :service="selectedServiceFromScheduler"
                                @onFetch="e => fetchingSchedules(e)" @selectedService="e => selectedService(e)"
                ...
                <div class="pwfv-finalfields">
                    <div class="hidden pwfv-recaptcha-parent" v-show="false">
                        <div id="recaptcha" v-show="currentPageIndex == form.pages.length - 1" class="g-recaptcha"
                            :data-sitekey="siteKey"></div>
                    </div>
                    <button @click="beforePageChange(-1)" v-if="currentPageIndex != 0"><i v-html="icons.arrowLeft"></i>
                        Prev</button>
                    ...
                    <button @click="submit()" :disabled="isSubmitting" v-if="currentPageIndex == form.pages.length - 1"
                        class="pwfv-submit">Submit</button>
                </div>
            </div>
        </div>
